...
lookahead_k    = 1.5;   % adaptive lookahead multiplier
psi_filter_tau = 0.67;  % heading filter time constant [s]
...
ki_psi     = 0.05;
kd_psi     = 0.5;
RUDDER_MAX = deg2rad(40.0);  % rad

%% ===== MISI =====
u_des       = 1.5;   % kecepatan surge yang diinginkan [m/s]
arrive_dist = 0.5;   % jarak arrival [m]
goal_tol    = 4.0;   % jarak untuk speed ramp [m]
fade_in_sec = 2.0;   % waktu fade-in [s]

%% ===== LOCAL PLANNER: SAPF =====
USE_SAPF = true;   % <<< toggle ON/OFF

% Parameter SAPF (berbasis jurnal)
dsafe = 1.0;     % jarak aman minimum [m]
dvort = 2.0;     % jarak orbit obstacle [m]
Qstar = 3.0;     % radius pengaruh obstacle [m]
eta   = 3.0;     % gain obstacle
safe_dyn = 0.8;   % jarak aman tambahan [m]

% Obstacle dinamis (kotak 0.45 m)
obs_size = 0.45;

obs_dyn1.pos = [15, 6.5];    % naik (y bertambah)
obs_dyn1.vel = [0, 0.3];

obs_dyn2.pos = [37, 34];   % turun (y berkurang)
obs_dyn2.vel = [0, -0.3];
...
%% ===================================================================
%% 4. OBSTACLE TAMBAHAN — muncul setelah kapal melewati waypoint pertama
%% ===================================================================
% Format: [cx_m, cy_m, radius_m]
extraObs_gt_m = [35.2 19.3 0.25];   % obstacle muncul setelah waypoint 1

% Radius waypoint-pass threshold: dianggap melewati WP saat jarak < ini [m]
wp_pass_dist    = arrive_dist + 1.0;   % 3 m
extra_obs_done  = false;               % flag agar hanya replan sekali
obstacles_all_m = obstacles_static_m;  % kumulatif (untuk visualisasi & clearance)
...
LOG = zeros(N_max, 19); % tambah 2 kolom jarak obstacle dinamis
LOG_replan = false(N_max, 1);
obs_dyn_log = zeros(N_max, 4); % [x1 y1 x2 y2]
k_end      = N_max;
...
obs_dyn = [obs_dyn1.pos; obs_dyn2.pos];
    ...
    % ---- Cek obstacle tambahan: muncul setelah waypoint pertama ----
    if ~extra_obs_done && norm(pos - waypoint_m) < wp_pass_dist
        fprintf('[Replan] t=%.1f s | Kapal melewati WP1 → obstacle baru (%.1f,%.1f) m muncul\n', ...
            t, extraObs_gt_m(1), extraObs_gt_m(2));

        % Update occupancy grid dengan obstacle baru
        r_occ_new = max(extraObs_gt_m(3) + inflate_m, min_occ_m);
        for row = 1:nR
            for col = 1:nC
                cx_c = (col-0.5)*cell_m; cy_c = (row-0.5)*cell_m;
        ...
        % Replan: dari posisi saat ini langsung ke goal (tanpa lewat waypoint)
        seg_start_re = m2g(pos);
        seg_goal_re  = m2g(goal_m);
        path_g_re    = dstar_lite_plan(occ, nR, nC, seg_start_re, seg_goal_re);
        ...
        if ~isempty(path_g_re)
            ...
            % Re-smooth
            obstacles_all_m  = [obstacles_static_m; extraObs_gt_m];
            ...
            % simpan path replan untuk Figure 6
            path_smooth_replan = path_smooth_re;
            did_replan         = true;

            % Reset ILOS setelah replan (sama dengan ilos.reset() Python)
            sigma      = 0.0;
            psi_d_filt = psi;
            ilos_init  = true;
            ...
            warning('[Replan] Path tidak ditemukan → tetap pakai path lama');
        end
        extra_obs_done = true;
    end
    ...
    % ==== SAPF LOCAL OBSTACLE AVOIDANCE ====
if USE_SAPF
    obs_vel = [obs_dyn1.vel; obs_dyn2.vel];

[Fx_sapf, Fy_sapf] = compute_sapf_force(pos, obs_dyn, obs_vel, eta, Qstar, dsafe, dvort, obs_size, safe_dyn);

    if norm([Fx_sapf Fy_sapf]) > 1e-6
        psi_sapf = atan2(Fy_sapf, Fx_sapf);

        % blending global (ILOS) + lokal (SAPF)
% ==== hitung jarak minimum ke obstacle ====
d_list = vecnorm(obs_dyn - pos, 2, 2) - (obs_size/2 + safe_dyn);
d_min = min(d_list);

% ==== adaptive weight ====
w_sapf = max(0, min(1, (Qstar - d_min)/Qstar));

% optional: biar cepat balik ke path
w_sapf = w_sapf^2;

psi_avoid = atan2(Fy_sapf, Fx_sapf);

psi_error_avoid = wrap_angle(psi_avoid - psi_des);

% BATASI pengaruh SAPF (ini kunci stabilitas)
psi_correction = max(-deg2rad(30), min(deg2rad(30), psi_error_avoid));

psi_des = wrap_angle(psi_des + w_sapf * psi_correction);
    ...
    % ==== COLLISION CHECK ====
for i = 1:size(obs_dyn,1)
    if norm(pos - obs_dyn(i,:)) < (obs_size/2 + safe_dyn)
        fprintf('COLLISION dengan obstacle dinamis ke-%d pada t=%.2f s\n', i, t);
    end
end
...
h_extra_fill  = fill(cx_e + r_e*cos(th), cy_e + r_e*sin(th), ...
    'm','FaceAlpha',0.3,'EdgeColor','none','Visible','off','HandleVisibility','off');
h_extra_guard = plot(cx_e + r_e_guard*cos(th), cy_e + r_e_guard*sin(th), ...
    'm--','LineWidth',0.8,'Visible','off','HandleVisibility','off');
...
% --- Obstacle dinamis (kotak) ---
h_dyn1 = rectangle('Position',[obs_dyn1.pos(1)-obs_size/2, ...
                               obs_dyn1.pos(2)-obs_size/2, ...
                               obs_size, obs_size], ...
                   'FaceColor','r','EdgeColor','k');

h_dyn2 = rectangle('Position',[obs_dyn2.pos(1)-obs_size/2, ...
                               obs_dyn2.pos(2)-obs_size/2, ...
                               obs_size, obs_size], ...
...
for k = 1:frame_step:length(t_log)
    ...
    % ==== UPDATE VISUAL OBSTACLE DINAMIS ====
x1 = obs_dyn_log(k,1);
y1 = obs_dyn_log(k,2);
x2 = obs_dyn_log(k,3);
y2 = obs_dyn_log(k,4);
...
%% ===================================================================
if did_replan && ~isempty(path_smooth_replan)
...
%% --- Figure 7: Jarak ke Obstacle Dinamis ---
figure('Name','Fig 7: Jarak Obstacle Dinamis','Position',[300 200 900 450]);
hold on; grid on; box on;

plot(t_log, dist_dyn1, 'r', 'LineWidth', 1.5);
plot(t_log, dist_dyn2, 'b', 'LineWidth', 1.5);

yline(obs_size, 'k--', 'Collision Boundary');

xlabel('t [s]');
ylabel('Jarak [m]');
title('Jarak USV ke Obstacle Dinamis');

legend('Obstacle 1 (x=15 naik)', ...
       'Obstacle 2 (x=37 turun)', ...
       'Collision limit');
%% ===================================================================
%%                      HELPER FUNCTIONS
%% ===================================================================
...
function path_g = dstar_lite_plan(occ, nR, nC, start_g, goal_g, w)
    if nargin < 6, w = 1.52; end   % bobot heuristik ε (w=1 → optimal)
...
function [Fx_sapf, Fy_sapf] = compute_sapf_force(pos, obs_dyn, obs_vel, eta, Qstar, dsafe, dvort, obs_size, safe_dyn)

Fx_sapf = 0;
Fy_sapf = 0;

for i = 1:size(obs_dyn,1)
    dx = pos(1) - obs_dyn(i,1);
    dy = pos(2) - obs_dyn(i,2);
    % ==== velocity repulsion ====
v_rel = -obs_vel(i,:);   % relatif obstacle
F_vel = 0.5 * v_rel(:);  % gain kecil
d_center = sqrt(dx^2 + dy^2);
R_safe = obs_size/2 + safe_dyn;
d = d_center - R_safe;   % jarak ke "permukaan aman"

    if d < Qstar && d > 0

        % --- Repulsive ---
       d_eff = max(d, 0.1); % hindari singularity
F_rep = eta * (1/Qstar - 1/d_eff) * (1/d_eff^2) * [dx; dy];

        % --- Transisi SAPF (repulsive → vortex) ---
        if d <= dsafe
            gamma = pi/2;
        elseif d >= dvort
            gamma = 0;
        else
            gamma = (d - dsafe)/(dvort - dsafe) * (pi/2);
        end
